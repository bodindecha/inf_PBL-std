		...
		await ajax(cv.API_URL+"information", {type: "group", act: "title"}).then(async function(dat) {
			if (typeof dat.isGrouped !== "undefined" && !dat.isGrouped) initialRender([null, null, 0]); else {
				if (dat.score) {
					...
					$('main .page[path="group/information"] .score [name="net"]')
						.val(dat.score)
						.attr("class", "color-"+(" rrrog"[dat.score]));
					$('main .page[path="group/information"] .score [name="ppr"]').val(dat.score_paper != null ? dat.score_paper+" " : "ไม่มี");
					$('main .page[path="group/information"] .score [name="pst"]').val(dat.score_poster != null ? dat.score_poster+" " : "ไม่มี");
					$('main .page[path="group/information"] .score [name="act"]').val(dat.score_activity != null ? dat.score_activity+" " : "ไม่มี");
				} else {
					$('main .page[path="group/information"] .score').fadeOut();
					$('main .page[path="group/information"] .score [name="net"]')
						.val("").removeAttr("class");
					$('main .page[path="group/information"] .score:where([name="ppr"], [name="pst"], [name="act"])').val("");
